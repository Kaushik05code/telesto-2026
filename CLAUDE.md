...
Sheet layout (BMT tab): row 1 title · row 2 = publish checkboxes C2:U2 — **19 rounds, 0-based: column C = Round 0 … U = Round 18** ("SHOW ROUND ON WEBSITE", V2 = live-round indicator, V = TOTAL (LIVE)) · row 3 headers · rows 4–34 = 31 teams. `roundNum` is 0-based; `null` (not 0) means nothing live. Scores may be decimals. Website totals count **only checked rounds**; `roundNum` = highest checked round. Team names come from column B (blank → "Team N"). Sheet text is untrusted input — keep `esc()` on every rendered string.
